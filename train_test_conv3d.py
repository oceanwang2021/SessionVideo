...
from sklearn.metrics import ConfusionMatrixDisplay, accuracy_score, plot_confusion_matrix, precision_score, f1_score, recall_score, confusion_matrix, classification_report
...
my_model = conv3d_model.Conv3dModel(num_classes=num_classes, clip_len=clip_len, image_size=image_size)
my_model = my_model.to(device)
summary(my_model, (1, clip_len, image_size, image_size), batch_size=batch_size, device='cuda')
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        cm = confusion_matrix(targets, predicted, normalize='true', labels=list(num2name.keys()))

        plt.figure(figsize=(30, 30), dpi=100) 

        disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=list(num2name.values()))
        ...
        plt.ylabel(None, fontsize=40) #纵轴标签
        plt.xlabel(None, fontsize=40) # 横轴标签

        plt.tight_layout()
        plt.savefig(os.path.join('./save_cm', 'BUAA-CST2022', f'{model_type}_cm_{epoch}.png'))
        plt.savefig(os.path.join('./save_cm', 'BUAA-CST2022', f'{model_type}_cm_{epoch}.svg'))
        ...
        # if f1 > evaluation['f1']:
        #     evaluation['f1'] = f1
        #     torch.save(my_model.state_dict(), f'{model_type}_f1_model.pth')
